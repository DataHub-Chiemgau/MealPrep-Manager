...
export async function POST(req: NextRequest) {
  ...
  const { name, unit, category, pricePerUnit, stockQuantity } = await req.json();
  if (!name || !unit) {
  ...
  try {
    const ingredient = await prisma.baseIngredient.create({
      data: { name, unit, category: category || null, pricePerUnit: pricePerUnit ?? 0, stockQuantity: stockQuantity ?? 0 },
    });
